...
{
  if (b < 0 || e < 0 || m <= 0){
    printf ("exiting in rsa_modExp:b=%lld,e=%lld,m=%lld\n",b,e,m);
    exit(1);
  }
  b = b % m;
  if(e == 0) return 1;
...
void rsa_gen_pub_key(struct public_key_class *pub,long long modulus, long long e)
{
     pub->modulus = modulus;
     pub->exponent = e;
}

void rsa_gen_priv_key(struct private_key_class *priv, long long modulus, long long e)
{
     priv->modulus = modulus;
     priv->exponent = e;
}

void rsa_gen_keys_pq(struct public_key_class *pub, struct private_key_class *priv, int p, int q)
{
...
/*
  for(i=0;i<message_size;i++){
      printf("in rsa_encrypt, message[%d]=%llu\n",i,(unsigned long long)message[i]);
  }
*/

  for(i=0; i < message_size; i++){
 //   printf("calling rsa_modExp,i=%d,msg[i]=%llu\n",i,(unsigned long long)message[i]);
    encrypted[i] = rsa_modExp((unsigned long long)message[i], pub->exponent, pub->modulus);
  }
  //printf("returning rsa_encrypt=%p\n",encrypted);
  return encrypted;
}
